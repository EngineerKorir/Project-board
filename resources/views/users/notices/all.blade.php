  ...
  <section id="notices-all">
    <div class="card">
      ...
      <div class="card-block p-0">
        @if(empty($data))
        <p>You haven't published any notices.</p>
        @else
            ...
            <tr>
              <td width="2%" class="hidden-xs-down">
                <label class="custom-control custom-checkbox">
                  <input type="checkbox" name="select[]" value="{{ $item->id }}" class="custom-control-input">
                  <span class="custom-control-indicator"></span>
                </label>
              </td>
              <td class="pt-3">
                <h3><a href="#">{{ $item->title }}</a></h3>
                <div class="posted mt-2 mb-1 font-size-xs msg-date"><span class="date mr-2">{{ $item->created_at }}</span> <span class="views mr-2">Views: {{ $item->views }}</span>
                  <span class="comments">Comments: 0</span></div>
